...
export function WorktreeModal({ repoRoot, entries, branches, busy, worktreeError, onClose }: WorktreeModalProps) {
    ...
    // Update path suggestion when the effective branch changes
    const effectiveBranch = addMode === 'existing' ? selectedBranch : newBranchName.trim();
    useEffect(() => {
        if (effectiveBranch) {
    ...
    const handleAdd = () => {
        ...
        if (!branch || !destPath) return;
        vscode.postMessage({
            type: 'addWorktree',
            payload: { repoRoot, branch, createNew: addMode === 'new', worktreePath: destPath }
        });
        setNewBranchName('');
    };

    const handleRemoveRequest = (path: string) => {
    ...
    const handleRemoveConfirm = (path: string, force: boolean) => {
        ...
        vscode.postMessage({
            type: 'removeWorktree',
            payload: { repoRoot, path, force }
        });
    };

    const handleOpenInWindow = (path: string) => {
        vscode.postMessage({
            type: 'openWorktreeInWindow',
            payload: { path }
        });
    };

    const addBranchValid = (addMode === 'existing' ? !!selectedBranch : newBranchName.trim().length > 0)
        ...
        <div className="modal-backdrop" onClick={handleBackdropClick}>
            <div className="modal worktree-modal" role="dialog" aria-modal="true" aria-label="Worktree Manager">
                    ...
                    {entries.length === 0 ? (
                        ...
                        <ul className="worktree-list" aria-label="Worktree entries">
                            {entries.map((entry) => {
                                ...
                                return (
                                    <li key={entry.path} className="worktree-entry">
                                        <div className="worktree-entry__info">
                                            ...
                                            <span className="worktree-entry__path" title={entry.path}>
                                            ...
                                            {isPendingRemove && !isForceConfirm && (
                                                <div className="worktree-inline-confirm">
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={() => handleRemoveConfirm(entry.path, false)}
                                                        disabled={busy}
                                                    >
                                                        Remove
                                                    </button>
                                            ...
                                            {isForceConfirm && (
                                                <div className="worktree-inline-confirm worktree-inline-confirm--force">
                                                    ...
                                                    <button
                                                        type="button"
                                                        className="worktree-action-btn worktree-action-btn--danger"
                                                        onClick={() => handleRemoveConfirm(entry.path, true)}
                                                        disabled={busy}
                                                    >
                                                        Force Remove
                                                    </button>
                                            ...
                                            {!isPendingRemove && !isForceConfirm && (
                                                <>
                                                    ...
                                                    <button
                                                        ...
                                                        disabled={busy || entry.isMain}
                                                        title={entry.isMain ? 'Cannot remove the main worktree' : 'Remove worktree'}
                                                    >
                ...
                <div className="worktree-add-form">
                    ...
                    {addMode === 'existing' ? (
                        <select
                            className="worktree-add-form__select"
                    ...
                    <button
                        type="button"
                        className="worktree-add-form__btn"
                        onClick={handleAdd}
                        disabled={busy || !addBranchValid}
                    >
                        {busy ? (
                            <>
